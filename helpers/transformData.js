...
export const formatAddress = (addressObj) => {
  return (addressObj?.city_name || "") + ", " + (addressObj?.state_name || "");
};

export const getBrand = (attributesArr) => {
  let brand = "No definida";
  for (let i = 0; i < attributesArr.length; i++) {
    if (attributesArr[i]?.id && attributesArr[i]?.id == "BRAND") {
      brand = attributesArr[i]?.value_name || "No definada";
      break;
    }
  }
